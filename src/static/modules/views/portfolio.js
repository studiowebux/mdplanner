      ...
      Array.from(placeholders).map(async (el) => {
        ...
        try {
          ...
          const releaseHtml = release
            ? `<a class="github-release-badge" href="${this.escapeHtml(release.htmlUrl)}" target="_blank" rel="noopener noreferrer" data-stop-propagation>${this.escapeHtml(release.tagName)}</a>`
            : "";
          el.innerHTML = `
