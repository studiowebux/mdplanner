...
export class DependenciesModule {
  ...
  handleInput(e) {
    ...
    if (filteredTasks.length > 0) {
      filteredTasks.forEach((task) => {
        ...
        option.className =
          "px-3 py-2 cursor-pointer hover:bg-tertiary border-b border-default last:border-b-0";
        option.innerHTML = `
          <div class="font-medium text-primary">${task.title}</div>
          <div class="text-xs text-muted">${task.id} - ${task.section}</div>
        `;
  ...
  updateSelected() {
    ...
    this.tm.selectedDependencies.forEach((taskId) => {
      ...
      chip.className =
        "inline-flex items-center px-2 py-1 rounded-full text-xs border border-strong bg-tertiary text-primary";
      chip.innerHTML = `
        <span>${task ? task.title : taskId}</span>
        <button type="button" class="ml-1 text-secondary hover:text-primary" onclick="taskManager.dependenciesModule.remove('${taskId}')">
