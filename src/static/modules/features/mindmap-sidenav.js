...
export class MindmapSidenavModule {
  ...
  bindEvents() {
    // Close button
    document.getElementById('mindmapSidenavClose')?.addEventListener('click', () => {
      this.close();
    });

    // Cancel button
    document.getElementById('mindmapSidenavCancel')?.addEventListener('click', () => {
      this.close();
    });

    // Delete button
    document.getElementById('mindmapSidenavDelete')?.addEventListener('click', () => {
      this.handleDelete();
    });

    // Form submit
    document.getElementById('mindmapSidenavForm')?.addEventListener('submit', async (e) => {
      e.preventDefault();
      await this.save();
    });

    // Auto-save on input changes
    document.getElementById('mindmapSidenavTitle')?.addEventListener('input', () => {
      if (this.editingMindmapId) {
        this.scheduleAutoSave();
      }
    });

    document.getElementById('mindmapSidenavStructure')?.addEventListener('input', () => {
      this.updatePreview();
      if (this.editingMindmapId) {
        this.scheduleAutoSave();
      }
    ...
    document.getElementById('mindmapSidenavStructure')?.addEventListener('keydown', (e) => {
      this.handleKeyDown(e);
    });

    // Toolbar buttons
    document.getElementById('mmSidenavAddRoot')?.addEventListener('click', () => this.addRoot());
    document.getElementById('mmSidenavAddChild')?.addEventListener('click', () => this.addChild());
    document.getElementById('mmSidenavAddSibling')?.addEventListener('click', () => this.addSibling());
    document.getElementById('mmSidenavIndent')?.addEventListener('click', () => this.indent());
    document.getElementById('mmSidenavUnindent')?.addEventListener('click', () => this.unindent());
    document.getElementById('mmSidenavMoveUp')?.addEventListener('click', () => this.moveLine(-1));
    document.getElementById('mmSidenavMoveDown')?.addEventListener('click', () => this.moveLine(1));
    document.getElementById('mmSidenavDeleteLine')?.addEventListener('click', () => this.deleteLine());
  }
  ...
  openEdit(mindmapId) {
    const mindmap = this.tm.mindmapModule.mindmaps.find(m => m.id === mindmapId);
    if (!mindmap) return;

    this.editingMindmapId = mindmapId;

    // Update header
    document.getElementById('mindmapSidenavHeader').textContent = 'Edit Mindmap';
  ...
  fillForm(mindmap) {
    ...
    document.getElementById('mindmapSidenavStructure').value = this.convertNodesToStructure(mindmap.nodes || []);
  }
  ...
  async handleDelete() {
    ...
    const mindmap = this.tm.mindmapModule.mindmaps.find(m => m.id === this.editingMindmapId);
    if (!mindmap) return;
