...
export class C4SidenavModule {
  ...
  bindEvents() {
    document.getElementById('c4SidenavClose')?.addEventListener('click', () => this.close());
    document.getElementById('c4SidenavCancel')?.addEventListener('click', () => this.close());
    document.getElementById('c4SidenavDelete')?.addEventListener('click', () => this.handleDelete());

    // Auto-save on input changes
    const inputs = ['c4SidenavName', 'c4SidenavLevel', 'c4SidenavType',
                    'c4SidenavTechnology', 'c4SidenavDescription',
                    'c4SidenavX', 'c4SidenavY'];
    inputs.forEach(id => {
      document.getElementById(id)?.addEventListener('input', () => this.scheduleAutoSave());
      document.getElementById(id)?.addEventListener('change', () => this.scheduleAutoSave());
    });

    // Level change updates default type
    document.getElementById('c4SidenavLevel')?.addEventListener('change', (e) => {
      if (!this.editingComponentId) {
        document.getElementById('c4SidenavType').value = this.getDefaultTypeForLevel(e.target.value);
      }
    });

    // Add connection button
    document.getElementById('c4SidenavAddConnection')?.addEventListener('click', () => {
  ...
  openNew(x = 100, y = 100, level = null) {
    ...
    if (this.tm.c4NavigationStack && this.tm.c4NavigationStack.length > 0) {
      ...
      if (currentParent) {
        ...
        targetLevel = currentIndex < levels.length - 1 ? levels[currentIndex + 1] : 'code';
      }
    }
  ...
  openEdit(component) {
    ...
    document.getElementById('c4SidenavHeader').textContent = 'Edit C4 Component';
  ...
  fillForm() {
    document.getElementById('c4SidenavName').value = this.currentComponent.name || '';
    document.getElementById('c4SidenavLevel').value = this.currentComponent.level || 'context';
    document.getElementById('c4SidenavType').value = this.currentComponent.type || this.getDefaultTypeForLevel(this.currentComponent.level);
    document.getElementById('c4SidenavTechnology').value = this.currentComponent.technology || '';
    document.getElementById('c4SidenavDescription').value = this.currentComponent.description || '';
    document.getElementById('c4SidenavX').value = this.currentComponent.position?.x || 100;
    document.getElementById('c4SidenavY').value = this.currentComponent.position?.y || 100;
  ...
  renderConnections() {
    ...
    if (connections.length === 0) {
      container.innerHTML = '<div class="text-gray-400 dark:text-gray-500 text-sm italic py-2">No connections defined</div>';
      return;
    }
      ...
      return `
        <div class="flex items-center gap-2 py-1 group bg-gray-50 dark:bg-gray-700 rounded px-2">
          <div class="flex-1 min-w-0">
            <div class="text-sm text-gray-700 dark:text-gray-300 truncate">${escapeHtml(targetDisplay)}</div>
            <div class="text-xs text-gray-500 dark:text-gray-400 truncate">${escapeHtml(conn.label || 'No label')}</div>
          </div>
  ...
  getTargetDisplayName(target) {
    const component = this.tm.c4Components.find(c => c.id === target || c.name === target);
      ...
      if (matches.length > 0 && value.length > 0) {
        dropdown.innerHTML = matches.map(comp => `
          <div class="c4-dropdown-item px-3 py-2 hover:bg-gray-100 dark:hover:bg-gray-600 cursor-pointer text-sm" data-id="${comp.id}" data-name="${comp.name}">
            ${escapeHtml(comp.name)} <span class="text-xs text-gray-400">(${comp.level})</span>
          </div>
        `).join('');
  ...
  editConnection(index) {
    ...
    targetItem.innerHTML = `
      <div class="c4-conn-input w-full space-y-2">
        <div class="relative">
          <input type="text" class="c4-conn-target form-input text-sm" value="${escapeHtml(conn.target)}" autocomplete="off">
          <div class="c4-conn-dropdown hidden absolute top-full left-0 right-0 bg-white dark:bg-gray-700 border border-gray-300 dark:border-gray-600 rounded-b-md max-h-32 overflow-y-auto z-50 shadow-lg"></div>
        </div>
        <input type="text" class="c4-conn-label form-input text-sm" value="${escapeHtml(conn.label || '')}">
        <div class="flex gap-2">
  ...
  getDefaultTypeForLevel(level) {
    switch (level) {
      case 'context': return 'System';
      case 'container': return 'Container';
      case 'component': return 'Component';
      case 'code': return 'Class';
  ...
  async save() {
    ...
    try {
      if (this.editingComponentId) {
        ...
        const index = this.tm.c4Components.findIndex(c => c.id === this.editingComponentId);
        if (index !== -1) {
        ...
        if (this.tm.c4NavigationStack && this.tm.c4NavigationStack.length > 0) {
          data.parent = this.tm.c4NavigationStack[this.tm.c4NavigationStack.length - 1];
  ...
  async handleDelete() {
    ...
    const component = this.tm.c4Components.find(c => c.id === this.editingComponentId);
    if (!component) return;

    if (!confirm(`Delete "${component.name}"? This will also remove any children and connections.`)) return;
