...
export class BillingSidenavModule {
  ...
  bindEvents() {
    document.getElementById('billingSidenavClose')?.addEventListener('click', () => this.close());
    document.getElementById('billingSidenavCancel')?.addEventListener('click', () => this.close());
    document.getElementById('billingSidenavDelete')?.addEventListener('click', () => this.handleDelete());
  }
  ...
  openEditCustomer(customerId) {
    const customer = this.tm.billingModule?.customers.find(c => c.id === customerId);
    if (!customer) return;
  ...
  openEditRate(rateId) {
    const rate = this.tm.billingModule?.billingRates.find(r => r.id === rateId);
    if (!rate) return;
  ...
  openEditInvoice(invoiceId) {
    const invoice = this.tm.billingModule?.invoices.find(i => i.id === invoiceId);
    if (!invoice) return;
  ...
  openPanel(title) {
    ...
    this.renderContent();
    document.getElementById('billingSidenavDelete').classList.toggle('hidden', !this.editingId);
    ...
    return `
      <div class="sidenav-section">
        <div class="form-group">
          ...
          <input type="text" id="billingSidenavCustomerName" class="form-input" value="${escapeHtml(c.name || '')}" required>
        </div>
        <div class="sidenav-grid">
          <div class="form-group">
            <label class="form-label">Email</label>
            <input type="email" id="billingSidenavCustomerEmail" class="form-input" value="${escapeHtml(c.email || '')}">
          </div>
          <div class="form-group">
            <label class="form-label">Phone</label>
            <input type="tel" id="billingSidenavCustomerPhone" class="form-input" value="${escapeHtml(c.phone || '')}">
          </div>
        </div>
        <div class="form-group">
          <label class="form-label">Company</label>
          <input type="text" id="billingSidenavCustomerCompany" class="form-input" value="${escapeHtml(c.company || '')}">
        </div>
      </div>
      <div class="sidenav-section">
        <div class="sidenav-section-title">Billing Address</div>
        <div class="form-group">
          <input type="text" id="billingSidenavCustomerStreet" class="form-input" placeholder="Street" value="${escapeHtml(c.billingAddress?.street || '')}">
        </div>
        <div class="sidenav-grid grid-cols-3">
          <div class="form-group">
            <input type="text" id="billingSidenavCustomerCity" class="form-input" placeholder="City" value="${escapeHtml(c.billingAddress?.city || '')}">
          </div>
          <div class="form-group">
            <input type="text" id="billingSidenavCustomerState" class="form-input" placeholder="State" value="${escapeHtml(c.billingAddress?.state || '')}">
          </div>
          <div class="form-group">
            <input type="text" id="billingSidenavCustomerPostal" class="form-input" placeholder="Postal" value="${escapeHtml(c.billingAddress?.postalCode || '')}">
          </div>
        </div>
        <div class="form-group">
          <input type="text" id="billingSidenavCustomerCountry" class="form-input" placeholder="Country" value="${escapeHtml(c.billingAddress?.country || '')}">
        </div>
      </div>
      <div class="sidenav-section">
        <div class="form-group">
          <label class="form-label">Notes</label>
          <textarea id="billingSidenavCustomerNotes" class="form-input" rows="3">${escapeHtml(c.notes || '')}</textarea>
        </div>
      </div>
    `;
  }
    ...
    return `
      <div class="sidenav-section">
        <div class="form-group">
          ...
          <input type="text" id="billingSidenavRateName" class="form-input" value="${escapeHtml(r.name || '')}" required placeholder="e.g., Senior Developer">
        </div>
        <div class="sidenav-grid">
          <div class="form-group">
            <label class="form-label">Hourly Rate ($)</label>
            <input type="number" id="billingSidenavRateHourly" class="form-input" value="${r.hourlyRate || 0}" min="0" step="0.01">
          </div>
          <div class="form-group">
            <label class="form-label">Assignee</label>
            <input type="text" id="billingSidenavRateAssignee" class="form-input" value="${escapeHtml(r.assignee || '')}" placeholder="Optional">
          </div>
        </div>
        <div class="form-group">
          <label class="flex items-center gap-2 cursor-pointer">
            <input type="checkbox" id="billingSidenavRateDefault" class="rounded" ${r.isDefault ? 'checked' : ''}>
    ...
    return `
      <div class="sidenav-section">
        <div class="form-group">
          ...
          <input type="text" id="billingSidenavQuoteTitle" class="form-input" value="${escapeHtml(q.title || '')}" required>
        </div>
        <div class="sidenav-grid">
          <div class="form-group">
            ...
            <select id="billingSidenavQuoteCustomer" class="form-input">
              ...
              ${customers.map(c => `<option value="${c.id}" ${c.id === q.customerId ? 'selected' : ''}>${escapeHtml(c.name)}</option>`).join('')}
            </select>
          </div>
          <div class="form-group">
            <label class="form-label">Valid Until</label>
            <input type="date" id="billingSidenavQuoteValidUntil" class="form-input" value="${q.validUntil || ''}">
          </div>
        </div>
      </div>
      <div class="sidenav-section">
      ...
      <div class="sidenav-section">
        <div class="sidenav-grid">
          <div class="form-group">
            ...
            <input type="number" id="billingSidenavQuoteTaxRate" class="form-input" value="${q.taxRate || 0}" min="0" max="100" step="0.01"
      ...
      <div class="sidenav-section">
        <div class="form-group">
          <label class="form-label">Notes</label>
          <textarea id="billingSidenavQuoteNotes" class="form-input" rows="3">${escapeHtml(q.notes || '')}</textarea>
        </div>
      </div>
    `;
  }
    ...
    return `
      <div class="sidenav-section">
        <div class="form-group">
          ...
          <input type="text" id="billingSidenavInvoiceTitle" class="form-input" value="${escapeHtml(inv.title || '')}" required>
        </div>
        <div class="sidenav-grid">
          <div class="form-group">
            <label class="form-label">Customer *</label>
            <select id="billingSidenavInvoiceCustomer" class="form-input">
              <option value="">Select customer...</option>
              ${customers.map(c => `<option value="${c.id}" ${c.id === inv.customerId ? 'selected' : ''}>${escapeHtml(c.name)}</option>`).join('')}
            </select>
          </div>
          <div class="form-group">
            <label class="form-label">Due Date</label>
            <input type="date" id="billingSidenavInvoiceDueDate" class="form-input" value="${inv.dueDate || ''}">
          </div>
        </div>
      </div>
      <div class="sidenav-section">
      ...
      <div class="sidenav-section">
        <div class="sidenav-grid">
          <div class="form-group">
            <label class="form-label">Tax Rate (%)</label>
            <input type="number" id="billingSidenavInvoiceTaxRate" class="form-input" value="${inv.taxRate || 0}" min="0" max="100" step="0.01"
                   onchange="taskManager.billingSidenavModule.updateTotals()">
          </div>
          <div class="form-group">
            <label class="form-label">Subtotal</label>
      ...
      <div class="sidenav-section">
        <div class="form-group">
          <label class="form-label">Notes</label>
          <textarea id="billingSidenavInvoiceNotes" class="form-input" rows="3">${escapeHtml(inv.notes || '')}</textarea>
        </div>
      </div>
    `;
  }
  ...
  renderLineItems() {
    ...
    return this.lineItems.map((item, idx) => `
      <div class="flex gap-2 items-center bg-gray-50 dark:bg-gray-700/50 p-2 rounded" data-line-idx="${idx}">
        ...
        <input type="number" placeholder="Qty" value="${item.quantity || 1}" step="0.01" min="0"
        ...
        <input type="number" placeholder="Rate" value="${item.rate || 0}" step="0.01" min="0"
        ...
        <span class="w-16 text-sm text-right text-gray-900 dark:text-gray-100">$${(item.amount || 0).toFixed(2)}</span>
  ...
  addLineItem() {
    this.lineItems.push({ id: crypto.randomUUID().substring(0, 8), description: '', quantity: 1, rate: 0, amount: 0 });
  ...
  getFormData() {
    switch (this.entityType) {
      case 'customer':
        return {
          name: document.getElementById('billingSidenavCustomerName')?.value.trim() || '',
          email: document.getElementById('billingSidenavCustomerEmail')?.value.trim() || null,
          phone: document.getElementById('billingSidenavCustomerPhone')?.value.trim() || null,
          company: document.getElementById('billingSidenavCustomerCompany')?.value.trim() || null,
          billingAddress: {
            street: document.getElementById('billingSidenavCustomerStreet')?.value.trim() || null,
            city: document.getElementById('billingSidenavCustomerCity')?.value.trim() || null,
            state: document.getElementById('billingSidenavCustomerState')?.value.trim() || null,
            postalCode: document.getElementById('billingSidenavCustomerPostal')?.value.trim() || null,
            country: document.getElementById('billingSidenavCustomerCountry')?.value.trim() || null
          },
          notes: document.getElementById('billingSidenavCustomerNotes')?.value.trim() || null
        };

      case 'rate':
        return {
          name: document.getElementById('billingSidenavRateName')?.value.trim() || '',
          hourlyRate: parseFloat(document.getElementById('billingSidenavRateHourly')?.value) || 0,
          assignee: document.getElementById('billingSidenavRateAssignee')?.value.trim() || null,
          isDefault: document.getElementById('billingSidenavRateDefault')?.checked || false
        };

      case 'quote':
        return {
          title: document.getElementById('billingSidenavQuoteTitle')?.value.trim() || '',
          customerId: document.getElementById('billingSidenavQuoteCustomer')?.value || null,
          validUntil: document.getElementById('billingSidenavQuoteValidUntil')?.value || null,
          taxRate: parseFloat(document.getElementById('billingSidenavQuoteTaxRate')?.value) || 0,
          lineItems: this.lineItems,
          notes: document.getElementById('billingSidenavQuoteNotes')?.value.trim() || null
        };

      case 'invoice':
        return {
          title: document.getElementById('billingSidenavInvoiceTitle')?.value.trim() || '',
          customerId: document.getElementById('billingSidenavInvoiceCustomer')?.value || null,
          dueDate: document.getElementById('billingSidenavInvoiceDueDate')?.value || null,
          taxRate: parseFloat(document.getElementById('billingSidenavInvoiceTaxRate')?.value) || 0,
          lineItems: this.lineItems,
          notes: document.getElementById('billingSidenavInvoiceNotes')?.value.trim() || null
        };
  ...
  async save() {
    ...
    try {
      if (this.editingId) {
        ...
        document.getElementById('billingSidenavDelete').classList.remove('hidden');
      }
  ...
  getRequiredFieldName() {
    switch (this.entityType) {
      ...
      case 'rate': return 'name';
      case 'quote': return 'title';
      case 'invoice': return 'title';
      default: return 'name';
    }
  }
