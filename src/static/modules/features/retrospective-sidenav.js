...
export class RetrospectiveSidenavModule {
  ...
  bindEvents() {
    // Close button
    document.getElementById('retroSidenavClose')?.addEventListener('click', () => {
      this.close();
    });

    // Cancel button
    document.getElementById('retroSidenavCancel')?.addEventListener('click', () => {
      this.close();
    });

    // Delete button
    document.getElementById('retroSidenavDelete')?.addEventListener('click', () => {
      this.handleDelete();
    });

    // Form submit
    document.getElementById('retroSidenavForm')?.addEventListener('submit', async (e) => {
      e.preventDefault();
      await this.save();
    });

    // Auto-save on input changes
    const inputs = ['retroSidenavTitle', 'retroSidenavDate', 'retroSidenavStatus',
  ...
  openNew() {
    ...
    // Update header
    document.getElementById('retroSidenavHeader').textContent = 'New Retrospective';
  ...
  openEdit(retroId) {
    ...
    // Update header
    document.getElementById('retroSidenavHeader').textContent = 'Edit Retrospective';
  ...
  clearForm() {
    ...
    document.getElementById('retroSidenavDate').value = new Date().toISOString().split('T')[0];
  ...
  fillForm(retro) {
    ...
    document.getElementById('retroSidenavContinue').value = (retro.continue || []).join('\n');
    document.getElementById('retroSidenavStop').value = (retro.stop || []).join('\n');
    document.getElementById('retroSidenavStart').value = (retro.start || []).join('\n');
  }
  ...
  async handleDelete() {
    ...
    const retro = this.tm.retrospectives.find(r => r.id === this.editingRetroId);
    if (!retro) return;
