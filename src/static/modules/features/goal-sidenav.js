...
export class GoalSidenavModule {
  ...
  bindEvents() {
    // Close button
    document.getElementById('goalSidenavClose')?.addEventListener('click', () => {
      this.close();
    });

    // Cancel button
    document.getElementById('goalSidenavCancel')?.addEventListener('click', () => {
      this.close();
    });

    // Delete button
    document.getElementById('goalSidenavDelete')?.addEventListener('click', () => {
      this.handleDelete();
    });

    // Form submit
    document.getElementById('goalSidenavForm')?.addEventListener('submit', async (e) => {
      e.preventDefault();
      await this.save();
    });

    // Auto-save on input changes
    const inputs = ['goalSidenavTitle', 'goalSidenavType', 'goalSidenavStatus',
