...
export const BrainsAPI = {
  ...
  async register(data) {
    return post("/api/brains", data);
  },

  async remove(name) {
    return del(`/api/brains/${encodeURIComponent(name)}`);
  },

  async update(name, data) {
    return patch(`/api/brains/${encodeURIComponent(name)}`, data);
  },

  async fetchFiles(name, path = "") {
    const q = path ? `?path=${encodeURIComponent(path)}` : "";
    const response = await get(
      `/api/brains/${encodeURIComponent(name)}/files${q}`,
    );
    return response.json();
  },

  async fetchFile(name, path) {
    const response = await get(
      `/api/brains/${encodeURIComponent(name)}/file?path=${encodeURIComponent(path)}`,
    );
    return response.text();
  },

  async fetchSessions(name) {
    const response = await get(
      `/api/brains/${encodeURIComponent(name)}/sessions`,
    );
    return response.json();
  },

  async fetchSession(name, id) {
    const response = await get(
      `/api/brains/${encodeURIComponent(name)}/sessions/${encodeURIComponent(id)}`,
    );
    return response.json();
  },

  async fetchMemory(name) {
    const response = await get(
      `/api/brains/${encodeURIComponent(name)}/memory`,
    );
    return response.json();
  },

  async saveMemory(name, content) {
    return put(`/api/brains/${encodeURIComponent(name)}/memory`, { content });
  },

  async setup(name, data) {
    return post(`/api/brains/${encodeURIComponent(name)}/setup`, data);
  },

  async rebuildRules(name) {
    return post(
      `/api/brains/${encodeURIComponent(name)}/rebuild-rules`,
      {},
    );
  },

  async fetchStacks() {
    const response = await get("/api/brains/stacks");
    return response.json();
  },

  async fetchPractices() {
    const response = await get("/api/brains/practices");
    return response.json();
  },

  async fetchWorkflows() {
    const response = await get("/api/brains/workflows");
    return response.json();
  },

  async syncDiff(from, to, dirs) {
    const params = new URLSearchParams({ from, to });
    if (dirs) params.set("dirs", dirs);
    const response = await get(`/api/brains/sync/diff?${params}`);
    return response.json();
  },

  async syncApply(data) {
    return post("/api/brains/sync/apply", data);
  },

  async fetchDirs(parent) {
    const response = await get(
      `/api/brains/dirs?parent=${encodeURIComponent(parent)}`,
    );
    return response.json();
  },
