...
export class TaskAPI {
  ...
  async handle(req: Request): Promise<Response> {
    ...
    try {
      ...
      if (
        method === "GET" && pathParts.length === 2 && pathParts[1] === "version"
      ) {
      ...
      if (
        ...
        pathParts[1] === "projects" && pathParts[2] === "switch"
      ) {
        const body = await req.json();
        const success = await this.projectManager.switchProject(body.filename);
        if (success) {
          return new Response(
            JSON.stringify({ success: true, filename: body.filename }),
            { headers },
          );
        }
      ...
      if (
        method === "POST" && pathParts.length === 3 &&
        pathParts[1] === "projects" && pathParts[2] === "create"
      ) {
        const body = await req.json();
      ...
      if (
        method === "GET" && pathParts.length === 2 && pathParts[1] === "ideas"
      ) {
      ...
      // POST /api/ideas
      if (
        method === "POST" && pathParts.length === 2 && pathParts[1] === "ideas"
      ) {
        const body = await req.json();
      ...
      // PUT /api/ideas/:id
      if (
        method === "PUT" && pathParts.length === 3 && pathParts[1] === "ideas"
      ) {
        const id = pathParts[2];
        const body = await req.json();
        const ideas = await this.parser.readIdeas();
      ...
      // GET /api/swot
      if (
        method === "GET" && pathParts.length === 2 && pathParts[1] === "swot"
      ) {
      ...
      // POST /api/swot
      if (
        method === "POST" && pathParts.length === 2 && pathParts[1] === "swot"
      ) {
        const body = await req.json();
        const swotAnalyses = await this.parser.readSwotAnalyses();
        const id = crypto.randomUUID().substring(0, 8);
      ...
      // PUT /api/swot/:id
      if (
        method === "PUT" && pathParts.length === 3 && pathParts[1] === "swot"
      ) {
        const id = pathParts[2];
        const body = await req.json();
        const swotAnalyses = await this.parser.readSwotAnalyses();
      ...
      // DELETE /api/swot/:id
      if (
        method === "DELETE" && pathParts.length === 3 && pathParts[1] === "swot"
      ) {
        const id = pathParts[2];
        const swotAnalyses = await this.parser.readSwotAnalyses();
      ...
      if (
        method === "GET" && pathParts.length === 2 &&
        pathParts[1] === "project-value-board"
      ) {
      ...
      if (
        method === "GET" && pathParts.length === 2 && pathParts[1] === "brief"
      ) {
      ...
      if (
        method === "POST" && pathParts.length === 2 && pathParts[1] === "brief"
      ) {
        const briefs = await this.parser.readBriefs();
        const body = await req.json();
        const newBrief = {
      ...
      if (
        method === "PUT" && pathParts.length === 3 && pathParts[1] === "brief"
      ) {
        const id = pathParts[2];
        const briefs = await this.parser.readBriefs();
      ...
      if (
        ...
        return new Response(JSON.stringify({ components: c4Components }), {
          headers,
        });
      }
      ...
      if (
        method === "POST" && pathParts.length === 4 &&
        pathParts[1] === "capacity" && pathParts[3] === "members"
      ) {
      ...
      if (
        ...
        const memberIndex = plan.teamMembers.findIndex((m) =>
          m.id === memberId
        );
        if (memberIndex === -1) {
        ...
        await this.parser.saveCapacityPlans(plans);
        return new Response(JSON.stringify(plan.teamMembers[memberIndex]), {
          headers,
        });
      }
      ...
      if (
        method === "DELETE" && pathParts.length === 5 &&
        pathParts[1] === "capacity" && pathParts[3] === "members"
      ) {
        const planId = pathParts[2];
        const memberId = pathParts[4];
      ...
      if (
        method === "POST" && pathParts.length === 4 &&
        pathParts[1] === "capacity" && pathParts[3] === "allocations"
      ) {
        const planId = pathParts[2];
        const body = await req.json();
        const plans = await this.parser.readCapacityPlans();
        ...
        await this.parser.saveCapacityPlans(plans);
        return new Response(JSON.stringify(newAllocation), {
          status: 201,
          headers,
        });
      }
      ...
      if (
        ...
        await this.parser.saveCapacityPlans(plans);
        return new Response(JSON.stringify(plan.allocations[allocIndex]), {
          headers,
        });
      }
      ...
      if (
        method === "GET" && pathParts.length === 4 &&
        pathParts[1] === "capacity" && pathParts[3] === "utilization"
      ) {
        const planId = pathParts[2];
        const plans = await this.parser.readCapacityPlans();
        const plan = plans.find((p) => p.id === planId);
        ...
        const utilization = plan.teamMembers.map((member) => {
          ...
          for (
            const alloc of plan.allocations.filter((a) =>
      ...
      if (
        method === "GET" && pathParts.length === 4 &&
        pathParts[1] === "capacity" && pathParts[3] === "suggest-assignments"
      ) {
        const planId = pathParts[2];
        const plans = await this.parser.readCapacityPlans();
        const plan = plans.find((p) => p.id === planId);
        ...
        const tasks = await this.parser.readTasks();
        const unassignedTasks = this.getUnassignedTasks(tasks).sort((a, b) =>
        ...
        for (const member of plan.teamMembers) {
          ...
          const allocated = plan.allocations
            .filter((a) =>
              a.memberId === member.id && a.weekStart === weekStart
            )
      ...
      if (
        method === "POST" && pathParts.length === 4 &&
        pathParts[1] === "capacity" && pathParts[3] === "apply-assignments"
      ) {
        const planId = pathParts[2];
        const body = await req.json();
        ...
        for (const suggestion of suggestions) {
          ...
          const member = plan.teamMembers.find((m) =>
            m.id === suggestion.memberId
          );
          if (member) {
        ...
        await this.parser.saveCapacityPlans(plans);
        return new Response(
          JSON.stringify({ success: true, applied: suggestions.length }),
          { headers },
        );
      }
      ...
      if (
        ...
        return new Response(JSON.stringify(newBuilder), {
          status: 201,
          headers,
        });
      }
      ...
      if (
        method === "POST" && pathParts.length === 4 &&
        pathParts[1] === "strategic-levels" && pathParts[3] === "levels"
      ) {
      ...
      if (
        ...
        await this.parser.saveStrategicLevelsBuilders(builders);
        return new Response(JSON.stringify(builder.levels[levelIndex]), {
          headers,
        });
      }
      ...
      // GET /api/quotes
      if (
        method === "GET" && pathParts.length === 2 && pathParts[1] === "quotes"
      ) {
      ...
      // GET /api/quotes/:id
      if (
        method === "GET" && pathParts.length === 3 && pathParts[1] === "quotes"
      ) {
        const id = pathParts[2];
        const quotes = await this.parser.readQuotes();
      ...
      // POST /api/quotes
      if (
        method === "POST" && pathParts.length === 2 && pathParts[1] === "quotes"
      ) {
        const body = await req.json();
        const quotes = await this.parser.readQuotes();
        const id = crypto.randomUUID().substring(0, 8);
      ...
      // PUT /api/quotes/:id
      if (
        method === "PUT" && pathParts.length === 3 && pathParts[1] === "quotes"
      ) {
        const id = pathParts[2];
        const body = await req.json();
        const quotes = await this.parser.readQuotes();
      ...
      if (
        method === "POST" && pathParts.length === 4 &&
        pathParts[1] === "quotes" && pathParts[3] === "send"
      ) {
        const id = pathParts[2];
        const quotes = await this.parser.readQuotes();
        const index = quotes.findIndex((q) => q.id === id);
      ...
      if (
        method === "POST" && pathParts.length === 4 &&
        pathParts[1] === "quotes" && pathParts[3] === "accept"
      ) {
        const id = pathParts[2];
        const quotes = await this.parser.readQuotes();
        const index = quotes.findIndex((q) => q.id === id);
      ...
      if (
        method === "POST" && pathParts.length === 4 &&
        pathParts[1] === "quotes" && pathParts[3] === "to-invoice"
      ) {
        const id = pathParts[2];
        const quotes = await this.parser.readQuotes();
        const quote = quotes.find((q) => q.id === id);
      ...
      if (
        method === "POST" && pathParts.length === 4 &&
        pathParts[1] === "invoices" && pathParts[3] === "send"
      ) {
        const id = pathParts[2];
        const invoices = await this.parser.readInvoices();
        const index = invoices.findIndex((inv) => inv.id === id);
      ...
      if (
        method === "GET" && pathParts.length === 4 &&
        pathParts[1] === "invoices" && pathParts[3] === "payments"
      ) {
      ...
      if (
        method === "POST" && pathParts.length === 3 &&
        pathParts[1] === "invoices" && pathParts[2] === "generate"
      ) {
        const body = await req.json();
        const { customerId, taskIds, startDate, endDate, hourlyRate, title } =
          body;

        if (!customerId || !taskIds || !Array.isArray(taskIds)) {
          return new Response(
            JSON.stringify({ error: "customerId and taskIds are required" }),
            { status: 400, headers },
          );
        }
      ...
      if (
        method === "GET" && pathParts.length === 3 &&
        pathParts[1] === "billing" && pathParts[2] === "summary"
      ) {
        const invoices = await this.parser.readInvoices();
        const quotes = await this.parser.readQuotes();
  ...
  private flattenTasksWithParent(
    tasks: Task[],
    parentId?: string,
...
<body>
  ...
  private async appendTasksToMarkdown(tasks: Task[]): Promise<number> {
    try {
      ...
      for (const [sectionName, sectionTasks] of tasksBySection) {
        const insertIndex = this.findOrCreateSectionInsertPoint(
          lines,
          sectionName,
        );
  ...
  private findOrCreateSectionInsertPoint(
    lines: string[],
    sectionName: string,
  ): number {
    ...
    if (sectionIndex === -1) {
      ...
      if (boardIndex !== -1) {
        ...
        while (
          insertPosition < lines.length && lines[insertPosition].trim() === ""
        ) {
          insertPosition++;
        }
      ...
      for (let j = sectionIndex + 1; j < lines.length; j++) {
        if (lines[j].startsWith("## ")) {
          ...
          while (
            insertIndex > sectionIndex && lines[insertIndex].trim() === ""
          ) {
            insertIndex--;
          }
  ...
  private generateTaskMarkdown(
    ...
    for (const task of tasks) {
      ...
      const configStr = configParts.length > 0
        ...
        : "";
      lines.push(
        `${indent}- ${checkbox} (${task.id}) ${task.title}${configStr}`,
      );
