...
// Meetings / Assigned / Vacation / Mentions always render (EmptyState fallback).
// ---------------------------------------------------------------------------

const PersonHeader: FC<{ person: Person; initials: string }> = (
  { person, initials },
) => (
  <header class="detail-header person-detail__header">
    <div class="person-detail__identity">
      ...
      <div>
        ...
        {person.title && <p class="person-detail__title">{person.title}</p>}
      </div>
    </div>
    {person.agentType && (
...
const PersonMeta: FC<{ person: Person; manager: Person | null }> = (
  { person, manager },
) => (
...
const SkillsSection: FC<{ person: Person }> = ({ person }) => {
  if (!person.skills || person.skills.length === 0) return null;
  return (
...
const AgentSection: FC<{ person: Person }> = ({ person }) => {
  if (!person.agentType || person.agentType === "human") return null;
  return (
    <section class="detail-section person-detail__section">
      <h2>Agent</h2>
      <dl class="person-detail__meta">
...
const AccountsSection: FC<{ person: Person }> = ({ person }) => {
  if (!person.accounts || Object.keys(person.accounts).length === 0) {
    return null;
  }
  return (
    <section class="detail-section person-detail__section">
...
const NotesSection: FC<{ person: Person }> = ({ person }) => {
...
const ReportsSection: FC<{ reports: Person[] }> = ({ reports }) => {
...
const RetrospectivesSection: FC<{ retrospectives: Retrospective[] }> = (
  { retrospectives },
) => {
...
const MeetingsSection: FC<{
  ...
  attendedMeetings: Meeting[];
  attendedMeetingsTotal: number;
}> = ({ person, attendedMeetings, attendedMeetingsTotal }) => (
  <section class="detail-section person-detail__section">
    <h2>
      Meetings
            ...
            {attendedMeetings.map((m) => (
              <li key={m.id}>
                ...
                <span class="person-detail__meeting-meta">
                  {formatDate(m.date)} &middot; {(m.attendees ?? []).length}
                  {" "}
                  attendees
                </span>
              </li>
...
const AnalyticsSection: FC<{ analytics?: PersonAnalytics }> = (
  { analytics },
) => {
...
const AssignedSection: FC<{
  assignedTasks: Task[];
  assignedGoals: Goal[];
  toggleHref: string;
  toggleLabel: string;
}> = ({ assignedTasks, assignedGoals, toggleHref, toggleLabel }) => (
  <section class="detail-section person-detail__section">
    <h2>
      Assigned
...
const VacationSection: FC<{ vacations: VacationRequest[] }> = (
  { vacations },
) => (
  <section class="detail-section person-detail__section">
    <h2>
...
const MentionsSection: FC<{
  person: Person;
  mentionsByType: Record<string, SearchResult[]>;
  mentionsTotal: number;
}> = ({ person, mentionsByType, mentionsTotal }) => (
  <section class="detail-section person-detail__section">
    <h2>
      Mentions
      {mentionsTotal > 0 && (
                ...
                <div key={type} class="person-detail__mention-group">
                  ...
                  <ul class="person-detail__mention-list">
                    {items.map((r) => (
                      <li key={r.id}>
                        <a
                          href={`${ENTITY_TYPE_ROUTES[type] ?? ""}/${r.id}`}
                        >
                          {r.title}
                        </a>
                      </li>
  ...
  return (
    ...
    >
      <main id="person-detail" class="detail-view person-detail">
        ...
        <PersonHeader person={person} initials={initials} />

        <ArchivedBanner entity={person} />

        <PersonMeta person={person} manager={manager} />

        <SkillsSection person={person} />

        <AgentSection person={person} />

        <AccountsSection person={person} />
        ...
        <VacationSection vacations={vacations} />
