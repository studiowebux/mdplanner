...
(function () {
  ...
  function getExportStyles() {
    return [
      ...
      ".task-timeline__row--header { font-weight: 600; font-size: 0.75rem; color: " +
      C.textMuted + "; }",
      ...
      ".task-timeline__label { flex-shrink: 0; width: 20rem; padding: 0.5rem 1rem; display: flex; align-items: center; gap: 0.5rem; border-right: 1px solid " +
      C.border + "; overflow: hidden; }",
      ".task-timeline__label--header { font-weight: 600; }",
      ".task-timeline__row-title { font-size: 0.875rem; font-weight: 500; color: " +
      C.textPrimary +
      "; text-decoration: none; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }",
      ".task-timeline__track { flex: 1; position: relative; min-height: 2rem; }",
      ".task-timeline__month-marker { position: absolute; top: 0; bottom: 0; font-size: 0.625rem; color: " +
      C.textSecondary +
      "; white-space: nowrap; padding-left: 0.25rem; border-left: 1px solid " +
      C.border + "; }",
      ".task-timeline__month-marker--quarter { font-weight: 700; color: " +
      C.textPrimary + "; border-left: 2px solid " + C.borderStrong + "; }",
      ".task-timeline__grid-line { position: absolute; top: 0; bottom: 0; border-left: 1px solid " +
      C.border + "; pointer-events: none; }",
      ".task-timeline__grid-line--quarter { border-left: 2px solid " +
      C.borderStrong + "; }",
      ...
      ".task-timeline__bar--p1 { background: " + C.errorBg +
      "; border-left: 2px solid " + C.error + "; }",
      ".task-timeline__bar--p2 { background: " + C.warningBg +
      "; border-left: 2px solid " + C.warning + "; }",
      ".task-timeline__bar--p3 { background: " + C.infoBg +
      "; border-left: 2px solid " + C.info + "; }",
      ".task-timeline__bar--p4 { background: " + C.bgTertiary +
      "; border-left: 2px solid " + C.borderStrong + "; }",
      ".task-timeline__bar--p5 { background: " + C.bgTertiary +
      "; border-left: 2px solid " + C.textMuted + "; }",
      ".task-timeline__bar--completed { opacity: 0.5; }",
      ".task-timeline__bar-dates { font-size: 0.625rem; color: " +
      C.textPrimary + "; white-space: nowrap; }",
      ...
      "; }",
      ".task-priority--2 { background: " + C.warningBg + "; color: " +
      C.warning + "; }",
      ".task-priority--3 { background: " + C.infoBg + "; color: " + C.info +
      "; }",
      ".task-priority--4 { background: " + C.bgTertiary + "; color: " +
      C.textSecondary + "; }",
      ".task-priority--5 { background: " + C.bgTertiary + "; color: " +
      C.textMuted + "; }",
      ".task-timeline__blocked { font-size: 0.625rem; font-weight: 600; padding: 0.125rem 0.375rem; border-radius: 9999px; background: " +
      C.errorBg + "; color: " + C.error + "; flex-shrink: 0; }",
  ...
  function captureTimeline() {
    ...
    var w = innerEl
      ? Math.max(rect.width, parseInt(innerEl.dataset.minWidth || "800")) + 40
      : rect.width + 40;
  ...
  function exportSVG() {
    ...
    link.download = "timeline-" + new Date().toISOString().split("T")[0] +
      ".svg";
    link.href = url;
    link.click();
  ...
  document.addEventListener("click", function (e) {
    if (
      e.target.id === "timelineExportSVG" ||
      e.target.closest("#timelineExportSVG")
    ) {
      exportSVG();
    }
    if (
      e.target.id === "timelineExportPDF" ||
      e.target.closest("#timelineExportPDF")
    ) {
      exportPDF();
    }
  });
