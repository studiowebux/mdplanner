...
- Milestones: GET /milestones no longer writes new milestone files on every
  request — this was causing duplicates via race conditions when multiple browser
  tabs or SSE connections triggered concurrent reads; task-referenced milestone
  names are now surfaced as virtual in-memory entries instead
...
- Search: selecting a task result now switches to the list view (not board) and
  opens the task sidenav directly; fallback default for unknown types also changed
  from "board" to "list"
...
- Portfolio: clearing all URL rows no longer silently preserves old URLs —
  `urls: []` is now sent explicitly so the JSON key is present; backend serialize
  also treats empty arrays the same as `undefined` (omitted from YAML)
- Portfolio: `githubRepo` field was missing from the POST /portfolio body mapping
  — it is now passed through on project creation
